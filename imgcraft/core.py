...
# Vẫn giữ lại phần khởi tạo node một lần để code gọn gàng
...
class Editor:
    def __init__(self, model_dir="/content/ComfyUI/models"):
        # KHÔNG tải trước bất kỳ mô hình nào ở đây
        self.model_dir = model_dir
        self.output_path = None

    def _clear_memory(self):
        """Hàm dọn dẹp bộ nhớ tích cực để chạy sau mỗi lần xử lý."""
        gc.collect()
    ...
    def process(self, image_path: str, target_width: int, target_height: int):
        # Hàm process nhận kích thước tùy chỉnh
        ...
        with torch.inference_mode():
            try:
                # Mô hình được tải lại từ đầu trong mỗi lần xử lý
                print("\n--- Loading models for this run (slower but stable) ---")
                ...
                del clip # Giải phóng ngay sau khi dùng
                ...
                del model # Giải phóng ngay sau khi dùng
                
                print("Decoding latents...")
                decoded_image = vae_decode_node.decode(vae, image_out_latent)[0]
                del vae # Giải phóng ngay sau khi dùng
                ...
                print("Cleaning up memory after run...")
                self._clear_memory()
